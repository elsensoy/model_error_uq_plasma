...
results_dir = os.path.join("..", "mcmc-results-11-26-24")
...
# Update mcmc_inference function
def mcmc_inference(logpdf, initial_sample, iterations=200, save_interval=10, base_path="mcmc-results-11-26-24"):
    setup_logger()
    ...
    os.makedirs(metrics_dir, exist_ok=True)  # Directory for saving JSON files
    ...
    with open(checkpoint_file, 'w') as f:
        f.write("iteration,v1_log,alpha_log,status,delayed_rejection,acceptance_rate\n")
        ...
        try:
            ...
            if posterior_value in [-np.inf]:
                ...
                with open(checkpoint_file, 'a') as f:
                    f.write(f"{iteration + 1},{v1_log},{alpha_log},Penalized,No DR,0.0\n")
            ...
            if simulation_result is not None:
                # Save simulation result as a JSON file
                metrics_file = os.path.join(metrics_dir, f"iteration_{iteration + 1}_metrics.json")
                with open(metrics_file, 'w') as metrics_f:
                    json.dump(simulation_result, metrics_f, indent=4)
                logging.info(f"Metrics saved for iteration {iteration + 1} at {metrics_file}")
...
def run_mcmc_with_optimized_params(json_path, observed_data, config, ion_velocity_weight=2.0, iterations=200):
    ...
    base_path = f"mcmc_samples_11_26"

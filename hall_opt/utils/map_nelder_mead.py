...
def run_simulation(config):
    ...
    # Passing a JSON file ensures compatibility with Julia's file-based input requirements
...
def handle_nan_values(data, placeholder="NaN"): 
    ...
    if isinstance(data, dict):
        ...
        return np.where(np.isnan(data) | np.isinf(data), placeholder, data).tolist() #should i include -inf as Nan? this might possibly lead to inconsistency in acceptance logic. idk.
...
def hallthruster_jl_wrapper(v1, v2, config, use_time_averaged=True, save_every_n_grid_points=None):
    """
    might need logic revision in handling invalid parameters.
    """

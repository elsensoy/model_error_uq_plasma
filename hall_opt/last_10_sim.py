...
# Skip the first column and extract v1 (column 2) and v2 (column 3)
v1_column = checkpoint_data.iloc[:, 1]
v2_column = checkpoint_data.iloc[:, 2]

# Traverse from the end to find the first 10 valid parameter pairs
valid_samples = []
for v1, v2 in zip(reversed(v1_column), reversed(v2_column)):
    valid_samples.append((v1, v2))
    if len(valid_samples) == 10:  # Stop once we find 10 valid samples
        break
...
for v1, v2 in valid_samples:
    # Run simulation using your wrapper

...
def save_results_to_json(result_dict, filename, save_every_n_grid_points=10):
    ...
    for key in spatial_keys:
        if key in result_dict:
            print(f"Original {key} data shape: {np.array(result_dict[key]).shape}")
            

            if key == 'z_normalized' and len(result_dict[key]) <= save_every_n_grid_points:
                print(f"{key} already subsampled. Skipping subsampling.")
            else:
                if key == 'z_normalized':
                    result_dict[key] = subsample_data(result_dict[key], save_every_n_grid_points)
                else:
                    result_dict[key] = [subsample_data(sublist, save_every_n_grid_points) for sublist in result_dict[key]]
            
            print(f"Subsampled {key} data shape: {np.array(result_dict[key]).shape}")
...
def run_multilogbohm_simulation(config, ion_velocity_weight, use_time_averaged=True, save_every_n_grid_points=10):
    """
    Run the MultiLogBohm simulation and return the ground truth with optional time-averaging.
    
    :param config: Configuration for the MultiLogBohm model.
    :param ion_velocity_weight: The weight for ion velocity.
    ...
    :param save_every_n_grid_points: This will only be used for subsampling during saving (not during time-averaging).
    ...
    # Step 2: Extract non-spatial (scalar) metrics like thrust and discharge current
    ...
    # Step 3: Extract spatial metrics (e.g., ion velocity, potential)
    if use_time_averaged:
...
def log_likelihood(simulated_data, observed_data, sigma=0.08, ion_velocity_weight=5.0):
...
def compute_neg_log_posterior(v_log, observed_data, config, ion_velocity_weight, sigma=0.08, save_every_n_grid_points=10):
    ...
    simulated_result = hallthruster_jl_wrapper(v1, v2, config, use_time_averaged=True, save_every_n_grid_points=save_every_n_grid_points)

    # Apply subsampling to the spatial metrics (e.g., ion_velocity, z_normalized)
    spatial_keys = ['ion_velocity']  # Ensure spatial metrics are handled correctly
    for key in spatial_keys:
        if key in simulated_result:
            print(f"Original {key} data shape (simulated):", np.array(simulated_result[key]).shape)
            if isinstance(simulated_result[key], list) and isinstance(simulated_result[key][0], list):
                # Subsample only if it's a list of lists (e.g., ion_velocity)
                simulated_result[key] = [subsample_data(sublist, save_every_n_grid_points) for sublist in simulated_result[key]]
            else:
                # Subsample a single 1D array like z_normalized
                simulated_result[key] = subsample_data(simulated_result[key], save_every_n_grid_points)
            print(f"Subsampled {key} data shape (simulated):", np.array(simulated_result[key]).shape)

    # Compute the log-likelihood, passing ion_velocity_weight
...
def callback(v_log, iteration_counter, config, use_time_averaged, save_every_n_grid_points, observed_data, ion_velocity_weight, loss_history):
    ...
    # Calculate current loss (negative log-posterior)
    ...
    # Save the iteration result along with the metrics and loss
    weight_str = str(ion_velocity_weight).replace('.', '_')
    save_map_iteration(v_log, iteration_counter[0], filename=f"w_{weight_str}_map_iteration_results.json")
    save_results_to_json(iteration_metrics, f'w_{weight_str}_iteration_metrics.json', save_every_n_grid_points)


def run_map_multiple_initial_guesses(observed_data, config, ion_velocity_weight=5.0, maxiter=100, save_every_n_grid_points=10):
    """

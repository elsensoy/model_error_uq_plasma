...
def load_json(filename):
    file_path = os.path.join("..", "results-Nelder-Mead", filename)  
    with open(file_path, 'r') as f:
        return json.load(f)

# Plotting loss values for multiple ion velocity weights on the same plot
def plot_loss_comparison(loss_files, labels, save_dir="nm_plots_comparison"):
    ...
    plt.title('Loss Comparison for Different Ion Velocity Weights(Nelder Mead)')
    plt.xlabel('Iteration')
    plt.ylabel('Loss Value')
    plt.grid(True)
    plt.legend()

    plt.savefig(os.path.join(save_dir, 'nm_loss_comparison.png'))
    plt.close()
    print(f"Loss comparison plot saved in {save_dir}/nm_loss_comparison.png")

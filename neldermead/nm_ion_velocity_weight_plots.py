...
def plot_ion_velocity_comparison_standardized(datasets, labels, save_dir="nm_plots_comparison"):
    ...
    # Find the global min and max ion velocity values for consistent y-axis scaling
    all_velocities = []
    for data in datasets:
        initial_guess_ion_velocity = np.array(data['initial_guess_data']['ion_velocity'][0])
        optimized_ion_velocity = np.array(data['optimized_data']['ion_velocity'][0])
        all_velocities.extend(initial_guess_ion_velocity)
        all_velocities.extend(optimized_ion_velocity)
    ...
    optimized_marker = 'x'  
    initial_guess_marker = 'o'  
    optimized_line_style = '-'  
    initial_guess_line_style = '--'  
    ...
    # Save the plot
    plt.savefig(os.path.join(save_dir, 'ion_velocity_comparison_standardized.png'))
    plt.close()
    print(f"Ion velocity comparison plot saved in {save_dir}/ion_velocity_comparison_standardized.png")



def plot_each_combined_weight(datasets, labels, save_dir="2plots_comparison"):
    os.makedirs(save_dir, exist_ok=True)
    for idx, data in enumerate(datasets):
        ...
        # Plot the initial guess with a label
        plt.plot(z_grid, initial_guess_ion_velocity, linestyle='--', linewidth=2, color=color, label=f'{labels[idx]} Initial Guess')
        plt.scatter(z_grid, initial_guess_ion_velocity, color=color, s=80, marker='o', alpha=0.8)

        # Plot the optimized values with a label
        plt.plot(z_grid, optimized_ion_velocity, linestyle='-', linewidth=3, color=color, label=f'{labels[idx]} Optimized')
        plt.scatter(z_grid, optimized_ion_velocity, color=color, s=100, marker='x')

        # Label the initial guesses and optimized values separately
        for i, (z, val_init, val_opt) in enumerate(zip(z_grid, initial_guess_ion_velocity, optimized_ion_velocity)):
            plt.text(z, val_init, f'{val_init:.1f}', fontsize=8, color=color, ha='right', va='bottom')
            plt.text(z, val_opt, f'{val_opt:.1f}', fontsize=8, color=color, ha='left', va='top')
        ...
        # Add a legend for both initial guesses and optimized values
        plt.legend(loc='best', fontsize=10)

        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'combined_initial_optimized_{labels[idx].replace(" ", "_")}.png'))
        plt.close()
        print(f"Combined plot saved for {labels[idx]} in {save_dir}/combined_initial_optimized_{labels[idx].replace(' ', '_')}.png")

def plot_ion_velocity_comparison(datasets, labels, save_dir="nm_plots_comparison"):
    os.makedirs(save_dir, exist_ok=True)
    ...
    for idx, data in enumerate(datasets):
        ...
        # Debugging: Print the actual ion velocities that are about to be plotted
        print(f"Plotting for {labels[idx]}")
        print(f"Initial Guess Ion Velocity: {initial_guess_data['ion_velocity'][0]}")
        print(f"Optimized Ion Velocity: {optimized_data['ion_velocity'][0]}")
...
def plot_each_optimized_value(datasets, labels, save_dir="nm_plots_comparison"):
    os.makedirs(save_dir, exist_ok=True)
...
def plot_each_initial_guess(datasets, labels, save_dir="nm_plots_comparison"):
    os.makedirs(save_dir, exist_ok=True)
    ...
    for idx, data in enumerate(datasets):
        ...
        # Add labels to each point in the initial guess
        for i in range(len(z_grid)):
            plt.text(z_grid[i], initial_guess_ion_velocity[i], f'{initial_guess_ion_velocity[i]:.1f}', 
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'initial_guess_with_labels_{labels[idx].replace(" ", "_")}.png'))
        plt.close()
        print(f"Initial guess plot saved for {labels[idx]} in {save_dir}/initial_guess_with_labels_{labels[idx].replace(' ', '_')}.png")
def plot_initial_guess_only(datasets, labels, save_dir="nm_plots_comparison"):
    os.makedirs(save_dir, exist_ok=True)
...
def plot_each_optimized_value(datasets, labels, save_dir="2nm_plots_comparison"):
    os.makedirs(save_dir, exist_ok=True)
...
def main():
    ...
    plot_ion_velocity_comparison_standardized(datasets, labels)

...
# Adjust path to include the directory where MCMCIterators is located
sys.path.append(os.path.abspath(".."))

# Path to results directory
RESULTS_DIR = os.path.join("..", "results-Nelder-Mead")

initial_guess_path = os.path.join(RESULTS_DIR, "best_initial_guess_w_2_0.json")
...
def mcmc_inference(logpdf, initial_sample, iterations=100, lower_bound=-5, upper_bound=3, save_interval=10, save_path=None):
    # make sure initial_sample is a numpy array
    ...
    initial_cov = 0.5 * np.eye(len(initial_sample))
    
    # DEBUG: Check initial_cov
    # print("\nDEBUG: Initial covariance matrix (initial_cov):")
    # print(f"Type: {type(initial_cov)}, Shape: {initial_cov.shape}")
